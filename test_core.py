...
def load_config() -> Dict[str, Any]:
    """Loads grading weights and thresholds from config.json. This is the first thing that runs!"""
    try:
...
def recompute_grades_for_students(students: List[Dict[str, Any]], config: Dict[str, Any]) -> None:
    """
    Recomputes final_grade and letter_grade for students after any score change.
    We just pass the list to the transform file functions.
    """
...
def load_section_data(filepath: str, config: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], List[Dict[str, str]]]:
    """
    Loads data from a CSV, validates it using ingest.py, and then computes grades.
    """
...
def get_existing_csv_files(directory: str = DATA_FOLDER) -> List[str]:
    """Auto-detects and lists all existing .csv files in the current folder."""
    return [f for f in os.listdir(directory) if f.endswith(".csv")]

# Helper for Conditional Score Styling 
def _get_score_style(score: Optional[float]) -> str:
    """Helper to apply color styling (green/red/yellow) based on a score value."""
    if score is None:
...
def display_students_table(
    ...
    """Displays students in a comprehensive, styled, and non-truncating Rich Table."""
    if not students:
...
def _get_next_student_id(students: List[Dict[str, Any]], section: str) -> str:
    """Generates the next unique student ID based on the section name (e.g., BSIT01, BSIT02)."""
...
def _prompt_score(prompt_text: str, allow_none: bool = True) -> Optional[float]:
    """Prompts the user for a score input (0-100) and handles validation and N/A inputs."""
    while True:
...
def _prompt_new_student(existing_students: List[Dict[str, Any]], pre_defined_section: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """Prompts for a single new student's details, including names, scores, and attendance."""
...
# Helper for saving a new Csv in menu 1
...
def export_entire_csv(records: List[Dict[str, Any]], path: str) -> str:
    ...
    Uses DEFAULT_FIELDS from reports.py.
    """
...
# Menu Handlers (basically ito po is yung core feats functions)

def handle_enter_new_data(config: Dict[str, Any], all_students: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Menu 1: Handles the creation of new student records with input loop and saving."""
...
def handle_load_existing_csv(config: Dict[str, Any]) -> None:
    """Menu 2: Loads a CSV file and displays a separate summary table for each section found."""
...
def handle_edit_existing_csv(config: Dict[str, Any], all_students: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Menu 3: Allows the user to select a CSV file and edit, add, or delete individual student records."""
    files = get_existing_csv_files()
    if not files:
        ...
        if edit_options == 'D':
            if Prompt.ask(f"[red]Are you sure you want to DELETE {student_to_edit['student_id']} (y/n)?[/red]", default='n') == 'y':
                ...
                try:
                    ...
                    # Already removed or not present, handle gracefully
                    pass
...
#as the func name suggstt its for generating reports
def handle_generate_reports(config: Dict[str, Any]) -> None:
    """Menu 5: Prompts user to select a CSV, loads only that data, and runs report generation and plotting."""
    files = get_existing_csv_files()
    if not files:
...
#function for grade curve
def handle_apply_curve(config: Dict[str, Any], all_students: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Menu 6: Loads a CSV, applies a grade curve based on a target mean, and saves the result to a NEW file."""
    files = get_existing_csv_files()
    if not files:
        CONSOLE.print("[yellow]No CSV files found to apply a grade curve to.[/yellow]")
        return all_students

    CONSOLE.print(Panel("[gold1]6. Apply Grade Curve[/gold1]", border_style="gold1"))
    for i, file in enumerate(files):
        CONSOLE.print(f"  [bold]{i+1}[/bold]: {file}")
    ...
    while True:
        # Generate the name for the curved grades
    ...
    # 7. Display results and seperaets per section
...
def main_menu() -> None:
    ...
    for _ in track(range(50), description="[gold1]Initializing EPSILOBN GRADING SYSTEM...[/gold1]", transient=True):
        time.sleep(0.04) 
    


    # !MAIN LOOP! (actual start)
    while True:
        CONSOLE.clear()
        ...
        if choice == '1':
            ...
            # Note: handle_edit_existing_csv updates the system list at tsala disk files
